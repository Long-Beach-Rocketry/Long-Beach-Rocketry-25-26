...
bool HwGpio::init(void)
{
    ...
    set_field(
        &base_addr_->MODER, uint32_t(settings_.mode), uint32_t(pin_num_), 2);
    set_field(
        &base_addr_->OTYPER, uint32_t(settings_.otype), uint32_t(pin_num_), 1);
    set_field(&base_addr_->OSPEEDR,
              uint32_t(settings_.ospeed),
              uint32_t(pin_num_),
              2);
    set_field(
        &base_addr_->PUPDR, uint32_t(settings_.pupd), uint32_t(pin_num_), 2);
