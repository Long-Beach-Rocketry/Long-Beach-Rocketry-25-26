...
static inline bool is_timer_12_to_17(TIM_TypeDef* t) {
  return (
    TIM12 == t || TIM13 == t || TIM14 == t ||
    TIM15 == t || TIM16 == t || TIM17 == t
  );
}
  ...
  */
  ...
  switch (channel) 
  {
    ...
    case PwmChannel::CH1:
      ...
      SetReg(&base_addr->CCMR1, static_cast<uint32_t>(settings.output_mode),
              TIM_CCMR1_OC1M_Pos, kTIM_CCMRx_OCxM_BitWidth
            );                                       // Config Output Mode
      base_addr->CCR1 = curr_duty_cycle;             // 0% Duty Cycle
      base_addr->CCMR1 |= TIM_CCMR1_OC1PE;           // Enable Preload 
      base_addr->CCER |= TIM_CCER_CC1E;              // Enable the output for Channel x
      break;
    ...
    case PwmChannel::CH3:
      ...
      SetReg(&base_addr->CCMR2, static_cast<uint32_t>(settings.output_mode),
