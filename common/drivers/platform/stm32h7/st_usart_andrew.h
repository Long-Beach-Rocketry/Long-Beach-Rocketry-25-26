...
#include "usart_andrew.h"  // Include the generic USART class header
...
class StUsartAndrew : public UsartAndrew
{
public:
    StUsartAndrew(const StUsartAndrewParams& params_);

    /**
     * @brief transmits data to serial output.
     * 
     * @param tx_buffer An uint8_t std::array data to be sent.
     * @return True on success. False, otherwise.
     */
    bool transmit(std::span<const uint8_t> tx_buffer) override;
