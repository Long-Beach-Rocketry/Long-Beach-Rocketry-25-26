...
namespace Stmh7
{
StUsartAndrew::StUsartAndrew(const StUsartAndrewParams& params_)
    : base_addr(params_.base_addr), mode(params_.mode)
{
    if (mode == OversampleMode::BY_8)
    {
        usartdiv = (2 * params_.sys_clock) / params_.baud_rate;
    }
    else
    {
        usartdiv = params_.sys_clock / params_.baud_rate;
    }
}

bool StUsartAndrew::transmit(std::span<const uint8_t> tx_buffer)
{
    size_t count = 0;
    ...
    return count == tx_buffer.size();
}

bool StUsartAndrew::receive(uint8_t& byte)
{
...
USART_TypeDef* StUsartAndrew::get_addr()
{
    return base_addr;
}
